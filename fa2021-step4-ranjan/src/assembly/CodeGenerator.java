...
public class CodeGenerator extends AbstractASTVisitor<CodeObject> {
	...
	protected CodeObject postprocess(AssignNode node, CodeObject left,
		...
		//Step 1a
		/*if (left.isVar()) {
			InstructionList leftLi = new InstructionList();
			leftLi = generateAddrFromVariable(left);
			co.code.add(new Label("here")); // remove this after debuggin
			co.code.addAll(leftLi); //Used to be codeobject
		}*/
	...
	* Generate code for a call expression
	...
	protected CodeObject postprocess(CallNode node, List<CodeObject> args) {
		...
		co.code.add(new Addi("sp", "-4", "sp"));	
		//step 3
		//co.code.add(new Label(generateFunctionOutLabel())); // this is wrong
		...
		co.temp = temp; //fixes the sw null(sw null 0(null)) issue but 0(null) is still a issue
		return co;
	}	
